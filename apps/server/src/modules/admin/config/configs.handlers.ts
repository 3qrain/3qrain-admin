...
import { invalidateConfig, mergeConfigDefault } from '~/services/config'
...
export async function getAll(c: Context) {
  ...
  for (const row of rows) {
    try { result[row.key] = mergeConfigDefault(row.key as ConfigKey, JSON.parse(row.value)) } catch { /* skip */ }
  }
...
export async function getByKey(c: Context) {
  ...
  return c.json(ok({ [key]: mergeConfigDefault(key as ConfigKey, JSON.parse(row.value)) }, '获取成功'), HttpStatusCodes.OK)
}
...
export async function update(c: Context) {
  ...
  const defaulted = mergeConfigDefault(key as ConfigKey, current)
  const merged = {
